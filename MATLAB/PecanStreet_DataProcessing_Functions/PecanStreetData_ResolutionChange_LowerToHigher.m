...
%% User Inputs - Resolution Information

OriginalResolution = 30; % Mins

NewResolution = 5; % Mins

ProcessingType = 2; % 1 - Full File to be Processed ; 2 - Part of the File to be Processed User Input is Required

%% Getting the Pre-Processed WeatherFile File

Weather_FileName='GarzasAdjuntas_1998_To_2017_WeatherData_NSRDB.csv'; % Enter File Name
...
NewResWeatherDataFileName=['GarzasAdjuntas_',num2str(StartYear),'_To_',num2str(EndYear),'_WeatherData_NSRDB_',num2str(OriginalResolution),'minTo', num2str(NewResolution) ,'minRes.csv'];

csvwrite(strcat(WeatherFile_FolderName,'/',NewResWeatherDataFileName),NewResFile);
